= Sanction: A Kick-access control Rails plugin

<b>if @you.has(:an_interest).over?(@a_tight_plugin)</b>
  "Keep reading" 
  else
...
It provides all of the back-end a Rails app requires for permissions management.
...
For the performance consience folks there are also the following functionally identical invocations that leverage ActiveRecord's eager loading capabilities.
