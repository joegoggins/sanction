= Sanction: A role based permissions managment system.
  # Declare the readers situation

  @user = User.find('you')
  @a_tight_plugin = BitchinPlugin.find_by_name("Sanction")

  @user.has(:an_interest).over?(@a_tight_plugin) ?
    "Keep reading" : 
    "Find a different plugin"

== How it works

Sanction works on the basis of principals and permissionables, also known as subject and object, or user and resource.  The plugins's DSL-ish syntax embodies this idea closely.  Whereas

A person might ask:
* <em>a user or a type of user</em>  
* <em>some capability</em>															 
* <em>the scope of the capability</em>

Sanction asks:
  @users = User.has(:some_capability).over(:some_resource)

  @users = User.has(
    :can_enjoy_programming, 
    :likes_coffee
  ).over(
    @nerdy_books,
    @lazy_saturdays
...
If you just need a true/false answer, throw a ? on the last method.
